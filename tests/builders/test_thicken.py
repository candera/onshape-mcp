...
class TestThickenBuilder:
    ...
    def test_build_with_basic_thickness(self):
        ...
        # Verify top-level wrapper structure
        assert result["btType"] == "BTFeatureDefinitionCall-1406"
        assert "feature" in result

        feature = result["feature"]
        assert feature["btType"] == "BTMFeature-134"
        assert feature["name"] == "BasicThicken"
        assert feature["featureType"] == "thicken"
    ...
    def test_build_complete_feature_with_all_options(self):
        ...
        feature = result["feature"]
        assert feature["btType"] == "BTMFeature-134"
        assert feature["name"] == "CompleteThicken"
        assert feature["featureType"] == "thicken"
        assert len(feature["parameters"]) == 6

        # Verify all parameters are present
        param_ids = [p["parameterId"] for p in feature["parameters"]]
